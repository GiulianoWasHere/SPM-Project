...
static inline bool writeToDisk(Task_t *in)
{
...
{ // must be multi-output
  ...
  L_Worker(std::vector<int> &vectorOfCount, size_t id, size_t numberOfTasks, size_t NumberOfLWorkers)
      : vectorOfCount(vectorOfCount), id(id), numberOfTasks(numberOfTasks), NumberOfLWorkers(NumberOfLWorkers) {}
  ...
  Task_t *svc(Task_t *in)
  {
    ...
    else //HERE WE WRITE IN THE FILE
    {
      if (compressing)
      {
        ...
        int val = vectorOfCount[idFile]++;
        //std::cout << "ID: " << id << "FILZA: " << in->filename << "\n"; 
        if (val >= in->nblocks - 1)
        {
...
{ // must be multi-input
  ...
  Task_t *svc(Task_t *in)
  {
    ...
    if (compressing) //***********COMPRESSING********
    {
      ...
      ff_send_out_to(in,(in->idFile + 1)% Lw);
    }
    else //***********DECOMPRESSING********
    {
...
int main(int argc, char *argv[])
{
  ...
  std::vector<int> vectorOfCount(FilesVector.size(),0);
  ...
  for (size_t i = 0; i < Lw; ++i)
  {
    if (overflowTasks)
    {
      LW.push_back(new ff::ff_comb(new MultiInputHelperNode, new L_Worker(vectorOfCount, i, numberTasks + 1, Lw)));
      overflowTasks--;
    }
    else
      LW.push_back(new ff::ff_comb(new MultiInputHelperNode, new L_Worker(vectorOfCount, i, numberTasks, Lw)));
  }
